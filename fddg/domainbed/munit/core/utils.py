...
def __write_images(image_outputs, display_image_num, file_name):
    ...
    vutils.save_image(image_grid, file_name+'.jpg')
    random_idx = 0
    for row_num in range(len(image_outputs)):
        tensor = image_outputs[row_num]
        if row_num == 0:
            save_name = file_name + 'original' + '_'
        elif row_num == 1:
            save_name = file_name + 'reconstruct' + '_'
        elif row_num >= 2:
            save_name = file_name + 'translate_' + str(random_idx) + '_'
            random_idx += 1
        for i in range(tensor.size(0)):
            torchvision.utils.save_image(tensor[i], save_name + str(i) + '.jpg')


def write_2images(image_outputs, display_image_num, image_directory, postfix):
    n = len(image_outputs)
    __write_images(image_outputs[0:n // 2], display_image_num, '%s/a2b_' % image_directory)
    __write_images(image_outputs[n // 2:n], display_image_num, '%s/b2a_' % image_directory)
...
def write_loss(iterations, trainer, train_writer):
    train_writer.add_scalar("loss_gen_total", trainer.module.loss_gen_total, iterations + 1)
    train_writer.add_scalar("loss_dis_total", trainer.module.loss_dis_total, iterations + 1)
...
def load_vgg16(model_dir):
    ...
    if not os.path.exists(os.path.join(model_dir, 'vgg16.weight')):
        if not os.path.exists(os.path.join(model_dir, 'vgg16.t7')):
            os.system('wget address -O ' + os.path.join(model_dir, 'vgg16.t7'))
